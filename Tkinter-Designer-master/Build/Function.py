...
# def remove_background(image):
#     try:
#         # Chuyển đổi ảnh đầu vào thành mảng byte
#         img_array = np.array(image)
#         _, img_encoded = cv2.imencode('.png', img_array)
#         img_bytes = img_encoded.tobytes()

#         # Gửi yêu cầu tới API DeepAI
#         api_url = "https://api.deepai.org/api/background-remover"
...
#         if response.status_code == 200:
#             # Lấy URL của ảnh đã xử lý từ API
#             output_url = response.json().get('output_url')
#             if not output_url:
#                 messagebox.showerror("Error", "Invalid output URL from API.")
#                 return image

#             # Tải ảnh từ output_url
#             removed_bg_img_data = requests.get(output_url).content
#             if not removed_bg_img_data:
#                 messagebox.showerror("Error", "Failed to fetch image data from output URL.")
#                 return image

#             # Giải mã dữ liệu ảnh bằng OpenCV
#             removed_bg_img_array = np.frombuffer(removed_bg_img_data, np.uint8)
#             removed_bg_img = cv2.imdecode(removed_bg_img_array, cv2.IMREAD_UNCHANGED)
#             if removed_bg_img is None:
#                 messagebox.showerror("Error", "Failed to decode the image.")
#                 return image

#             # Xử lý kênh màu
#             if removed_bg_img.shape[2] == 4:  # Nếu có kênh alpha
#                 # Chuyển từ BGRA sang RGBA (đúng thứ tự màu)
#                 return Image.fromarray(cv2.cvtColor(removed_bg_img, cv2.COLOR_BGRA2RGBA))
#             elif removed_bg_img.shape[2] == 3:  # Nếu chỉ có kênh màu RGB
#                 # Chuyển từ BGR (mặc định OpenCV) sang RGB
#                 return Image.fromarray(cv2.cvtColor(removed_bg_img, cv2.COLOR_BGR2RGB))
#             else:
#                 messagebox.showerror("Error", "Unexpected number of color channels.")
#                 return image
#         else:
#             messagebox.showerror("Background Removal Failed", f"Error {response.status_code}: {response.text}")
#             return image
#     except Exception as e:
