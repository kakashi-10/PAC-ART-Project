    ...
    if edited_image:
        ...
        if file_path:
            try:
                ...
                # Kết nối đến cơ sở dữ liệu để lưu thông tin hình ảnh
                print("Connecting to the database...")  # Debug output
                ...
                cursor = db.cursor()
                print("Connection successful.")  # Debug output

                # Chèn tên hình ảnh và đường dẫn vào bảng edited_image
                name_image = os.path.basename(file_path)  # Lấy tên hình ảnh từ đường dẫn
                sql = "INSERT INTO edited_image (name_image, image_path, user_id) VALUES (%s, %s, %s)"
                values = (name_image, file_path, user_session.user_id)  # Cung cấp đủ tham số
                print(f"Executing SQL: {sql} with values: {values}")  # Debug output
                cursor.execute(sql, values)
                db.commit()  # Lưu thay đổi
                print("Edited image added to database successfully.")  # Debug output

            except mysql.connector.Error as db_error:
                messagebox.showerror("Database Error", f"Error occurred while saving to the database: {db_error}")
                print(f"Database error while exporting image: {db_error}")  # Log database errors
            except Exception as e:
                messagebox.showerror("Export Failed", f"Error occurred while saving the image: {e}")
                print(f"Error while exporting image: {e}")  # Log any other errors
            finally:
...
def toggle_ai_textbox_and_dropdown():
    ...
    if window and window.winfo_exists():
        if not is_visible:
            ...
            action_button.place(x=1000, y=469)
